...
test_that("run_cyclum includes --run_number only when supplied (grid-search calling pattern)", {
  m <- mock_system2_capturing_args()
  on.exit(m$restore())
  local_mocked_bindings(py_config = function(...) list(python = "FAKE_PYTHON"), .package = "reticulate")

  run_cyclum(input_dir = "IN", encoder_width = c(50, 40, 30), epochs = 300,
             learning_rate = 1e-4, run_number = 5, proj_root = "PROOT")

  args <- m$captured$args
  expect_true(all(c("--run_number", "5") %in% args))
  idx <- which(args == "--encoder_width")
  expect_equal(args[(idx + 1):(idx + 3)], c("50", "40", "30"))
})

test_that("run_scPrisma builds the correct args and respects output_dir/n_top_genes/run_number", {
  m <- mock_system2_capturing_args()
  on.exit(m$restore())
  local_mocked_bindings(py_config = function(...) list(python = "FAKE_PYTHON"), .package = "reticulate")
  ...
  exit_code <- run_scPrisma(
    input_dir = "IN", output_dir = out_dir,
    regularisation_strength = 0.2, iternum = 100, n_top_genes = 3000, run_number = 2,
    proj_root = "PROOT"
  )
  ...
  expect_true(all(c("--regularisation_strength", "0.2", "--iternum", "100",
                     "--output_dir", out_dir, "--n_top_genes", "3000",
                     "--run_number", "2") %in% args))
  unlink(out_dir, recursive = TRUE)
})

test_that("run_scPrisma omits --output_dir/--n_top_genes/--run_number when not supplied", {
  m <- mock_system2_capturing_args()
  on.exit(m$restore())
  local_mocked_bindings(py_config = function(...) list(python = "FAKE_PYTHON"), .package = "reticulate")
  ...
  expect_false("--run_number" %in% args)
