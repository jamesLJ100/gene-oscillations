...
sim_file <- file.path(proj_root, "synthetic", "tests", "data", "c1000g2004_1_sim.rds")
...
# make_plots(sim$model, sim$counts, sim$sub_expression, fig_dir)
...
ggsave("synthetic/figures/mrna_trajectories.pdf", p_mrna, width = 10, height = 6, dpi = 300)
ggsave("synthetic/figures/premrna_trajectories.pdf", p_premrna, width = 10, height = 6, dpi = 300)
ggsave("synthetic/figures/protein_trajectories.pdf", p_protein, width = 10, height = 6, dpi = 300)

# Optional: Create a combined plot with all three panels
library(patchwork)
p_combined <- p_premrna / p_mrna / p_protein
ggsave("synthetic/figures/multi_modal_gene_trajectories.pdf", p_combined, width = 10, height = 14, dpi = 300)

cat(" All plots saved to 'synthetic/figures/'\n")
cat("   - mrna_trajectories.pdf\n")
cat("   - premrna_trajectories.pdf\n")
cat("   - protein_trajectories.pdf\n")
cat("   - multi_modal_gene_trajectories.pdf (combined)\n")
cat(" Simulation saved to 'synthetic/data/simulation_results.rds' (~200MB)\n")
...
pdf("synthetic/figures/expression_heatmap_time.pdf", width = 14, height = 10)
draw(ht)
dev.off()
draw(ht)
cat("  - expression_heatmap_time.pdf\n")
...
pdf("synthetic/figures/expression_heatmap_time_log1p.pdf", width = 14, height = 10)
draw(ht_log)
dev.off()
draw(ht_log)
cat("  - expression_heatmap_time_log1p.pdf\n")