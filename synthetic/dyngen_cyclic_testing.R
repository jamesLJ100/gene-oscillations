...
realnet_matrix <- readRDS(here::here("synthetic", "data", "realnet.rds"))
realcount_data <- readRDS(here::here("synthetic", "data", "realcount.rds"))
...
backbone_cycle_simple <- function() {
  ...
  expression_patterns <- tribble(
    ...
    "s0", "s1", "+A,+B,+C,+F", TRUE, TRUE, 50,
    "s1", "s2", "+D,-C,+E", FALSE, FALSE, 50,
    "s2", "s3", "+C,-A,-B", FALSE, FALSE, 50,
    "s3", "s1", "+A,+B,-D", FALSE, FALSE, 50
  )
...
run_simulation <- function(model_config) {
  ...
  model <- out$model
  # dataset$expression is log2(counts + 1); use the raw dataset$counts instead so
  # this feeds Cyclum/scPrisma un-normalized, same as everywhere else in this repo.
